    ...
    speedLimitTags.boundaries.forEach((boundary, index) => {
      ...
      if (tag && tag.speed_limit_tags_type === 'tag') {
        ...
        if (nextBoundary !== undefined) {
          ...
          ctx.fillStyle = color;
          ctx.strokeStyle = tag_name === 'UU' ? '#494641' : color;
          ctx.lineWidth = 1;
          ...
          const textWidth =
            ctx.measureText(tag_name).width +
            TEXT_RIGHT_PADDING +
          ...
          ctx.rect(
            x + textWidth,
            Y_POSITION,
            tagWidth - RECTANGLE_SPACING - textWidth,
            RECT_HEIGHT
          );
          ...
          if (tag_name === 'incompatible' || tag_name === 'missing_from_train') {
            const image = tag_name === 'incompatible' ? alertFillImage : questionImage;
            ...
            if (tag_name === 'incompatible' && cursor.x && cursor.y) {
              if (
            ...
            ctx.fillText(tag_name, textPosition, Y_POSITION + TEXT_PADDING_TOP + RECT_HEIGHT / 2);
          }
        }
      }
    });
