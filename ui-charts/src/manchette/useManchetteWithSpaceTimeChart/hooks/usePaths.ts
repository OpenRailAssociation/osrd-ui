import { useMemo } from 'react';

import { type PathLevel } from '../../../spaceTimeChart';
import { type ProjectPathTrainResult } from '../../Manchette';
import { PATH_COLOR_DEFAULT } from '../consts';

const transformCurve = (curve: ProjectPathTrainResult['points'][0], departureTime: Date) => {
  console.log(curve, departureTime);
  return curve.positions.map((position, i) => ({
    time: curve.times[i] + departureTime.getTime(),
    position,
  }));
};

const usePaths = (projectPathTrainResult: ProjectPathTrainResult[], selectedTrain?: number) =>
  useMemo(
    () =>
      projectPathTrainResult.flatMap((path) =>
        path.points.map<{
          id: string;
          label: string;
          color: string;
          level: PathLevel;
          points: { time: number; position: number }[];
        }>((points, ind) => ({
          id: `${path.id}-${ind}`,
          label: path.name,
          color: PATH_COLOR_DEFAULT,
          level: selectedTrain && selectedTrain === path.id ? 1 : 2,
          points: transformCurve(points, path.departureTime),
        }))
      ),
    [projectPathTrainResult, selectedTrain]
  );

export default usePaths;
